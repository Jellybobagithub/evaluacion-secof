...
const DIAS_NOMBRES_COMPLETOS = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
...
export default function Empleados() {
  ...
  const bajaMut = trpc.empleados.darDeBaja.useMutation({
    onSuccess: () => {
      utils.empleados.list.invalidate();
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {form.diaDescansoFijo !== null && (
                <p className="text-xs text-red-600">Descanso fijo: {DIAS_NOMBRES_COMPLETOS[form.diaDescansoFijo]}</p>
              )}
            </div>
                ...
                {[1, 2, 3, 4, 5, 6, 0].map(dia => {
                  ...
                  return (
                    <div key={dia} className="flex items-center gap-2">
                      ...
                      {esDescanso ? (
                        <span className="text-xs text-red-400 flex-1 italic">Descanso</span>
                      ) : (
                        <div className="flex items-center gap-1.5 flex-1">
                          <input type="time" value={h!.entrada}
                            onChange={e => setHora(dia, "entrada", e.target.value)}
                            className="h-7 px-2 text-xs rounded-lg border border-input bg-background focus:outline-none focus:ring-1 focus:ring-ring" />
                      ...
                      <button type="button" onClick={() => toggleDescanso(dia)}
                        className={"text-xs px-2 py-1 rounded-lg font-medium " + (esDescanso ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-red-100 text-red-600 hover:bg-red-200")}>
