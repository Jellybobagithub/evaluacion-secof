  ...
  kpiLider: router({
    ...
    resumenNivel2: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        // Última evaluación SECOF del mes + frecuencia semanal
        const { getDb } = await import('./db');
        const { evaluaciones } = await import('../drizzle/schema');
        const { eq, and, gte, lte, desc, sql: drizzleSql } = await import('drizzle-orm');
        const db = await getDb();
        let secof = null;
        let frecuenciaEval = { semanas: 0, de: 0 };
        if (db) {
          ...
          // Contar semanas del mes con al menos una evaluación completada
          ...
          const [[freqRow]] = await db.execute(drizzleSql`
            SELECT
              COUNT(DISTINCT YEARWEEK(fecha, 1)) as semanasConEval,
              CEIL(DATEDIFF(${ffStr}, ${fiStr}) / 7) as totalSemanas
            FROM evaluaciones
            WHERE sucursalId=${input.sucursalId}
              AND estado='completada'
              AND fecha >= ${fiStr} AND fecha <= ${ffStr}
          `) as any;
          // Calcular total semanas del mes (semanas ISO que tocan el mes)
          const [[totalRow]] = await db.execute(drizzleSql`
            SELECT COUNT(DISTINCT YEARWEEK(fecha, 1)) as total
            FROM (
              SELECT DATE_ADD(${fiStr}, INTERVAL seq DAY) as fecha
              FROM (SELECT 0 seq UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5
                ...
                UNION SELECT 26 UNION SELECT 27 UNION SELECT 28 UNION SELECT 29 UNION SELECT 30) nums
              WHERE DATE_ADD(${fiStr}, INTERVAL seq DAY) <= ${ffStr}
            ) cal
          `) as any;
          frecuenciaEval = {
            semanas: Number(freqRow?.semanasConEval ?? 0),
            de: Number(totalRow?.total ?? 0),
          };
        }
