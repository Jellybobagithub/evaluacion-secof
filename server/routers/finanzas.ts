...
export const finanzasRouter = router({
  ...
  comprasExternas: {
    ...
    guardar: protectedProcedure.input(z.object({sucursalId:z.number(),periodo:z.string(),fecha:z.string(),concepto:z.string(),proveedor:z.string().default('Externo'),cantidad:z.number().default(1),unidad:z.string().default('pz'),precioUnitario:z.number(),total:z.number(),notas:z.string().optional(),inv_productoId:z.number().optional()})).mutation(async({ctx,input})=>{const db=await getDb();if(!db)throw new TRPCError({code:'INTERNAL_SERVER_ERROR'});if(!['superadmin','owner','manager','leader'].includes(ctx.user.role))throw new TRPCError({code:'FORBIDDEN'});await db.execute(sql`INSERT INTO compras_externas(sucursalId,periodo,fecha,concepto,proveedor,cantidad,unidad,precioUnitario,total,notas,capturoId)VALUES(${input.sucursalId},${input.periodo},${input.fecha},${input.concepto},${input.proveedor},${input.cantidad},${input.unidad},${input.precioUnitario},${input.total},${input.notas??null},${ctx.user.id})`);if(input.inv_productoId){await db.execute(sql`INSERT INTO inv_movimientos(sucursalId,productoId,tipo,cantidadGramos,cantidadPiezas,referenciaTipo,notas)VALUES(${input.sucursalId},${input.inv_productoId},'entrada',0,${input.cantidad},'compra_externa',${input.concepto})`)}return{ok:true};}),
